                ...
                <div>
                    <span class="relative z-0 inline-flex rounded-md rtl:flex-row-reverse items-center gap-4">
                        <span>
                            ...
                            @if ($paginator->onFirstPage())
                                <span aria-disabled="true" aria-label="{{ __('pagination.previous') }}">
                                    <span
                                        class="relative inline-flex cursor-default items-center rounded-full border border-gray-300 outline-white outline bg-tictac-primary-orange px-2 py-2 text-sm font-medium leading-5 text-white"
                                        aria-hidden="true">
                            ...
                            @else
                                <button
                                    class="relative inline-flex items-center rounded-full cursor-pointer border border-gray-300 bg-tictac-primary-orange px-2 py-2 text-sm font-medium leading-5 text-white ring-blue-300 transition duration-150 ease-in-out "
                                    type="button" wire:click="previousPage('{{ $paginator->getPageName() }}')"
                        ...
                        @foreach ($elements as $element)
                            ...
                            @if (is_array($element))
                                @foreach ($element as $page => $url)
                                    <span
                                    @class([
                                        "text-tictac-primary-orange outline-white outline rounded-full",
                        ...
                        <span>
                            ...
                            @if ($paginator->hasMorePages())
                                <button
                                    class="relative cursor-pointer -ml-px inline-flex items-center  border bg-tictac-primary-orange border-gray-300 text-white rounded-full px-2 py-2 text-sm font-medium leading-5  ring-blue-300 transition duration-150 ease-in-out hover"
                                    type="button" wire:click="nextPage('{{ $paginator->getPageName() }}')"
                                    x-on:click="{{ $scrollIntoViewJsSnippet }}"
                            ...
                            @else
                                <span aria-disabled="true" aria-label="{{ __('pagination.next') }}">
                                    <span
                                        class="relative bg-tictac-primary-orange -ml-px inline-flex cursor-default items-center  border border-gray-300 text-white rounded-full px-2 py-2 text-sm font-medium leading-5 "
                                        aria-hidden="true">
                                        <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd"
                                                d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z"
