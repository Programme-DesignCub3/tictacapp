<?php

return [
    // Convert to multi dimensional array
    'home' => [
        'title' => 'Jelajahi Dunia Seru TicTacLand | Pilus TicTac Dua Kelinci',
        'description' => 'Bikin hari-harimu makin kriuk di TicTacLand! Pusat informasi snack pilus favorit, event TicTactivity, hingga update TicTalks yang seru. Masuk sekarang!',
    ],
    'product' => [
        'title' => 'TicTac Station | Pilih Varian Pilus Seru Teman Aktivitasmu',
        'description' => 'Temukan semua rasa favoritmu di TicTac Station. Dari pedas yang bikin melek sampai rasa original yang nagih, Pilus Dua Kelinci siap temani setiap momen serumu.',
    ],
    'event' => [
        'title' => 'TicTactivity | Cek Event Seru & Aktivitas Paling Gokil',
        'description' => 'Saatnya bergerak bersama TicTactivity! Ikuti berbagai event keren, tantangan seru, dan aktivitas seru lainnya yang bikin hari kamu makin renyah dan berenergi.'
    ],
    'blog' => [
        'title' => 'TicTalks | Info Lifestyle, Tips Seru, & Tren Snack Terbaru',
        'description' => 'Baca berita paling update di TicTalks. Dapatkan inspirasi gaya hidup, tips seru, hingga cerita di balik renyahnya Pilus TicTac Dua Kelinci di sini.'
    ]
];
